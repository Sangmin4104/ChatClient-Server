...
public class ClientThread extends Thread
{
                ...
                switch(command){
                    ...
                    case YES_POPMESSAGE : {
                        ...
                        try{
                            ...
                            if (roomNumber != 0){
                                ct_chatRoom.messages.append(id + " : " + msg + "\n");
                                if(id.equals(ct_logonID)){
                                    ct_chatRoom.message.setText("");
                                }
                                ct_chatRoom.message.requestFocusInWindow();
                                ...
                                TimerTask task = new TimerTask() {
                                    ...
                                    public void run() {
                                        ...
                                        if(!text.isEmpty()){
                                            if(text.equals(msg)) {
                                                String word = ct_chatRoom.messages.getText();
                                                String[] lines = word.split("\\n");
                                                String li = lines[lines.length -1];

                                                String[] p = li.split(":");
                                                StringBuilder rt = new StringBuilder();
                                                rt.append(p[0]);
                                                for(int i = 1; i < p.length; i++){
                                                    rt.append(": 메시지가 삭제되었습니다.\n");
                                                }
                                                lines[lines.length - 1] = rt.toString();
                                                word = String.join("\n", lines);
                                                ct_chatRoom.messages.setText(word);
                                            }
                                        }
                                    }
                                };
    ...
    public void requestPopMessage(String data){
        try {
            ct_buffer.setLength(0);
            ct_buffer.append(REQ_POPMESSAGE);
            ct_buffer.append(SEPARATOR);
            ct_buffer.append(ct_logonID);
            ct_buffer.append(SEPARATOR);
            ct_buffer.append(ct_roomNumber);
